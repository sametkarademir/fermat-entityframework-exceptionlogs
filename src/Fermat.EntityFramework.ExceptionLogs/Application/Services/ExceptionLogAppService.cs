...
namespace Fermat.EntityFramework.ExceptionLogs.Application.Services;
    ...
    : IExceptionLogAppService
{
    ...
    public async Task<int> CleanupOldExceptionLogsAsync(DateTime olderThan, CancellationToken cancellationToken = default)
    {
        var queryable = exceptionLogRepository.GetQueryable();
        ...
        while (countToDelete > totalDeleted)
        {
            try
            {
                ...
                await exceptionLogRepository.DeleteRangeAsync(exceptionLogsToDelete, cancellationToken: cancellationToken);
